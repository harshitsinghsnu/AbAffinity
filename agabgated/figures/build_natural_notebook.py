"""
build_natural_notebook.py
=========================
Builds BALM_AbAg_SAbDab_AbBind_SKEMPI_Analysis.ipynb with all experiments on the
natural antibody-antigen datasets (SAbDab / AbBind / SKEMPI / benchmark), using
mutual_strong.py results. Publication style: "Ours" labels, pK_d subscripts,
panel letters, 95% CI, Fisher tests, All-CDR vs mean-pool comparison.
"""
import json, os, uuid

HERE = os.path.dirname(os.path.abspath(__file__))
NB = os.path.join(HERE, 'BALM_AbAg_SAbDab_AbBind_SKEMPI_Analysis.ipynb')

cells = []
def md(t): cells.append({'cell_type':'markdown','metadata':{},'id':uuid.uuid4().hex[:8],'source':[t]})
def code(s): cells.append({'cell_type':'code','metadata':{},'id':uuid.uuid4().hex[:8],
                           'execution_count':None,'outputs':[],'source':[s]})

# ── Title ────────────────────────────────────────────────────────────────────
md("# Ours — Natural Antibody–Antigen Datasets\n"
   ...
   "10-fold CV on SAbDab/AbBind/SKEMPI, full-train-on-SAbDab→test-on-benchmark, PLM "
   "comparison, regression-head comparison, architecture ablations (Two-stream, "
   "Concat+MLP), antigen shuffling, gating ablation, mutational impact, and the "
   "**All-CDR (H1+H2+H3)** pooling variant compared against mean-pool ESM-2.")
...
# ── 3. Scatter true vs predicted (from fold predictions) ─────────────────────
...
# ── 5. Architecture comparison: Ours vs Two-stream vs Concat+MLP ────────────
...
# ── 6. Random antigen ablation ───────────────────────────────────────────────
md("---\n## 6. Antigen Importance — Real vs Shuffled (Random) Antigen\n"
   "Shuffling the antigen embedding quantifies reliance on antigen identity. "
   "**Multi-seed: mean ± std over 5 seeds.**")
code(
"ant={}\n"
"for ds in ['sabdab','abbind','skempi']:\n"
...
# ── 9. Mutational impact (SKEMPI / AbBind) ───────────────────────────────────
...
# ── 10. Gating ablation (saaintdb gate perturbation as reference) ────────────
...
# ── 11. ALL-CDR vs mean-pool (multi-seed, with error bars) ──────────────────
...
# ── Write notebook ───────────────────────────────────────────────────────────
nb = {'cells':cells,'metadata':{'kernelspec':{'display_name':'Python 3 (py310)',
      'language':'python','name':'py310'},'language_info':{'name':'python','version':'3.10'}},
