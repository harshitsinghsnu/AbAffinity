# 5GRJ -- Two-stream -- GT binding residues coloured by IG attribution
reinitialize
load pymol_groundtruth/pdb/pdb5grj.ent, 5GRJ
remove not polymer
remove 5GRJ and not chain H+L+A
hide everything
bg_color white
set ray_opaque_background, 1
set ray_shadows, 0
set ray_trace_mode, 1
set ray_trace_color, grey50
set antialias, 2
set cartoon_fancy_helices, 1
set cartoon_transparency, 0.25
set cartoon_side_chain_helper, 1
set stick_radius, 0.30
set sphere_scale, 0.34
set valence, 0
set label_size, 15
set label_font_id, 7
set label_color, black
set label_outline_color, white
set label_bg_color, white
set label_bg_transparency, 0.25
set float_labels, 1
set dash_color, grey30
set dash_width, 1.6
set dash_gap, 0.40
set dash_length, 0.40
set dash_radius, 0.045
set dash_round_ends, 0
show cartoon
color 0xC4E7BD, chain H
color 0xCBDEF0, chain L
color 0xCECEE5, chain A
show sticks, (5GRJ and chain H and resi 27) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 27) and name CA
color 0x2E974E, 5GRJ and chain H and resi 27
show sticks, (5GRJ and chain H and resi 28) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 28) and name CA
color 0x39A357, 5GRJ and chain H and resi 28
show sticks, (5GRJ and chain H and resi 31) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 31) and name CA
color 0x278F48, 5GRJ and chain H and resi 31
show sticks, (5GRJ and chain H and resi 33) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 33) and name CA
color 0x2B944B, 5GRJ and chain H and resi 33
show sticks, (5GRJ and chain H and resi 52) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 52) and name CA
color 0x56B567, 5GRJ and chain H and resi 52
show sticks, (5GRJ and chain H and resi 53) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 53) and name CA
color 0x41AB5D, 5GRJ and chain H and resi 53
show sticks, (5GRJ and chain H and resi 54) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 54) and name CA
color 0x38A256, 5GRJ and chain H and resi 54
show sticks, (5GRJ and chain H and resi 57) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 57) and name CA
color 0x50B264, 5GRJ and chain H and resi 57
show sticks, (5GRJ and chain H and resi 59) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 59) and name CA
color 0x53B365, 5GRJ and chain H and resi 59
show sticks, (5GRJ and chain H and resi 101) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 101) and name CA
color 0x59B769, 5GRJ and chain H and resi 101
show sticks, (5GRJ and chain H and resi 102) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 102) and name CA
color 0x7EC87E, 5GRJ and chain H and resi 102
show sticks, (5GRJ and chain H and resi 103) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 103) and name CA
color 0x70C274, 5GRJ and chain H and resi 103
show sticks, (5GRJ and chain H and resi 104) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 104) and name CA
color 0x76C477, 5GRJ and chain H and resi 104
show sticks, (5GRJ and chain H and resi 105) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 105) and name CA
color 0x7AC67B, 5GRJ and chain H and resi 105
show sticks, (5GRJ and chain H and resi 106) and (sidechain or name CA)
show spheres, (5GRJ and chain H and resi 106) and name CA
color 0x78C67A, 5GRJ and chain H and resi 106
show sticks, (5GRJ and chain L and resi 32) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 32) and name CA
color 0x4694C7, 5GRJ and chain L and resi 32
show sticks, (5GRJ and chain L and resi 34) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 34) and name CA
color 0x509BCB, 5GRJ and chain L and resi 34
show sticks, (5GRJ and chain L and resi 93) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 93) and name CA
color 0x5EA5D1, 5GRJ and chain L and resi 93
show sticks, (5GRJ and chain L and resi 97) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 97) and name CA
color 0x85BCDB, 5GRJ and chain L and resi 97
show sticks, (5GRJ and chain L and resi 99) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 99) and name CA
color 0x7BB7D9, 5GRJ and chain L and resi 99
show sticks, (5GRJ and chain L and resi 95) and (sidechain or name CA)
show spheres, (5GRJ and chain L and resi 95) and name CA
color 0x6BAED6, 5GRJ and chain L and resi 95
show sticks, (5GRJ and chain A and resi 54) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 54) and name CA
color 0x5A3394, 5GRJ and chain A and resi 54
show sticks, (5GRJ and chain A and resi 60) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 60) and name CA
color 0x8E8AC0, 5GRJ and chain A and resi 60
show sticks, (5GRJ and chain A and resi 61) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 61) and name CA
color 0x6950A2, 5GRJ and chain A and resi 61
show sticks, (5GRJ and chain A and resi 62) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 62) and name CA
color 0x684DA1, 5GRJ and chain A and resi 62
show sticks, (5GRJ and chain A and resi 63) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 63) and name CA
color 0x5A3495, 5GRJ and chain A and resi 63
show sticks, (5GRJ and chain A and resi 66) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 66) and name CA
color 0x562B90, 5GRJ and chain A and resi 66
show sticks, (5GRJ and chain A and resi 56) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 56) and name CA
color 0x6B54A4, 5GRJ and chain A and resi 56
show sticks, (5GRJ and chain A and resi 58) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 58) and name CA
color 0x6C56A6, 5GRJ and chain A and resi 58
show sticks, (5GRJ and chain A and resi 59) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 59) and name CA
color 0x5C3696, 5GRJ and chain A and resi 59
show sticks, (5GRJ and chain A and resi 69) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 69) and name CA
color 0x7F7CB9, 5GRJ and chain A and resi 69
show sticks, (5GRJ and chain A and resi 73) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 73) and name CA
color 0x5A3394, 5GRJ and chain A and resi 73
show sticks, (5GRJ and chain A and resi 75) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 75) and name CA
color 0x7363AC, 5GRJ and chain A and resi 75
show sticks, (5GRJ and chain A and resi 113) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 113) and name CA
color 0x66499F, 5GRJ and chain A and resi 113
show sticks, (5GRJ and chain A and resi 121) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 121) and name CA
color 0x7C75B5, 5GRJ and chain A and resi 121
show sticks, (5GRJ and chain A and resi 123) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 123) and name CA
color 0x6E59A7, 5GRJ and chain A and resi 123
show sticks, (5GRJ and chain A and resi 125) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 125) and name CA
color 0x5B3595, 5GRJ and chain A and resi 125
show sticks, (5GRJ and chain A and resi 115) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 115) and name CA
color 0x61409B, 5GRJ and chain A and resi 115
show sticks, (5GRJ and chain A and resi 117) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 117) and name CA
color 0x664AA0, 5GRJ and chain A and resi 117
show sticks, (5GRJ and chain A and resi 78) and (sidechain or name CA)
show spheres, (5GRJ and chain A and resi 78) and name CA
color 0x7E79B8, 5GRJ and chain A and resi 78
select interface, 5GRJ and ((chain H and resi 27+28+31+33+52+53+54+57+59+101+102+103+104+105+106) or (chain L and resi 32+34+93+97+99+95) or (chain A and resi 54+60+61+62+63+66+56+58+59+69+73+75+113+121+123+125+115+117+78))
deselect
set light_count, 2
set specular, 0.15
set ambient, 0.45

pseudoatom lab_H27, pos=[26.997, -74.746, 38.999], label="PHE27"
pseudoatom lab_H28, pos=[30.266, -76.107, 37.672], label="THR28"
pseudoatom lab_H31, pos=[30.779, -78.51, 32.133], label="SER31"
pseudoatom lab_H33, pos=[27.331, -75.146, 26.959], label="ILE33"
pseudoatom lab_H52, pos=[51.139, -75.306, 35.282], label="TYR52"
distance dl_H52, (5GRJ and chain H and resi 52 and name CA), lab_H52
hide labels, dl_H52
pseudoatom lab_H53, pos=[47.73, -75.786, 39.784], label="PRO53"
distance dl_H53, (5GRJ and chain H and resi 53 and name CA), lab_H53
hide labels, dl_H53
pseudoatom lab_H54, pos=[54.567, -76.694, 30.816], label="SER54"
distance dl_H54, (5GRJ and chain H and resi 54 and name CA), lab_H54
hide labels, dl_H54
pseudoatom lab_H57, pos=[57.965, -75.033, 26.29], label="ILE57"
distance dl_H57, (5GRJ and chain H and resi 57 and name CA), lab_H57
hide labels, dl_H57
pseudoatom lab_H59, pos=[14.282, -72.106, -8.881], label="PHE59"
distance dl_H59, (5GRJ and chain H and resi 59 and name CA), lab_H59
hide labels, dl_H59
pseudoatom lab_H101, pos=[22.607, -80.252, 31.204], label="LEU101"
pseudoatom lab_H102, pos=[40.97, -82.434, 48.9], label="GLY102"
distance dl_H102, (5GRJ and chain H and resi 102 and name CA), lab_H102
hide labels, dl_H102
pseudoatom lab_H103, pos=[20.146, -80.644, 26.349], label="THR103"
pseudoatom lab_H104, pos=[-0.538, -79.698, 10.874], label="VAL104"
distance dl_H104, (5GRJ and chain H and resi 104 and name CA), lab_H104
hide labels, dl_H104
pseudoatom lab_H105, pos=[-8.019, -76.487, 20.615], label="THR105"
distance dl_H105, (5GRJ and chain H and resi 105 and name CA), lab_H105
hide labels, dl_H105
pseudoatom lab_H106, pos=[-10.508, -75.909, 23.871], label="THR106"
distance dl_H106, (5GRJ and chain H and resi 106 and name CA), lab_H106
hide labels, dl_H106
pseudoatom lab_L32, pos=[10.938, -82.49, 17.284], label="TYR32"
pseudoatom lab_L34, pos=[11.999, -80.269, 22.132], label="TYR34"
pseudoatom lab_L93, pos=[-3.046, -77.28, 14.095], label="TYR93"
distance dl_L93, (5GRJ and chain L and resi 93 and name CA), lab_L93
hide labels, dl_L93
pseudoatom lab_L97, pos=[11.862, -78.326, -5.492], label="SER97"
distance dl_L97, (5GRJ and chain L and resi 97 and name CA), lab_L97
hide labels, dl_L97
pseudoatom lab_L99, pos=[4.363, -73.315, 4.214], label="ARG99"
distance dl_L99, (5GRJ and chain L and resi 99 and name CA), lab_L99
hide labels, dl_L99
pseudoatom lab_L95, pos=[17.205, -79.27, 11.81], label="SER95"
pseudoatom lab_A54, pos=[37.911, -83.737, 23.684], label="ILE54"
pseudoatom lab_A60, pos=[6.956, -84.236, 1.159], label="GLU60"
distance dl_A60, (5GRJ and chain A and resi 60 and name CA), lab_A60
hide labels, dl_A60
pseudoatom lab_A61, pos=[-5.48, -82.019, 17.455], label="ASP61"
distance dl_A61, (5GRJ and chain A and resi 61 and name CA), lab_A61
hide labels, dl_A61
pseudoatom lab_A62, pos=[20.252, -84.397, 23.647], label="LYS62"
pseudoatom lab_A63, pos=[2.0, -85.162, 7.713], label="ASN63"
distance dl_A63, (5GRJ and chain A and resi 63 and name CA), lab_A63
hide labels, dl_A63
pseudoatom lab_A66, pos=[31.124, -85.273, 26.878], label="GLN66"
pseudoatom lab_A56, pos=[31.243, -84.917, 22.235], label="TYR56"
pseudoatom lab_A58, pos=[9.46, -86.291, -2.068], label="GLU58"
distance dl_A58, (5GRJ and chain A and resi 58 and name CA), lab_A58
hide labels, dl_A58
pseudoatom lab_A59, pos=[21.576, -86.693, 19.166], label="MET59"
pseudoatom lab_A69, pos=[40.729, -84.927, 29.324], label="HIS69"
pseudoatom lab_A73, pos=[44.42, -86.007, 44.477], label="ASP73"
distance dl_A73, (5GRJ and chain A and resi 73 and name CA), lab_A73
hide labels, dl_A73
pseudoatom lab_A75, pos=[34.172, -85.389, 57.944], label="LYS75"
distance dl_A75, (5GRJ and chain A and resi 75 and name CA), lab_A75
hide labels, dl_A75
pseudoatom lab_A113, pos=[26.673, -86.012, 15.16], label="ARG113"
pseudoatom lab_A121, pos=[38.171, -79.42, 15.546], label="ALA121"
pseudoatom lab_A123, pos=[16.878, -83.243, -11.931], label="TYR123"
distance dl_A123, (5GRJ and chain A and resi 123 and name CA), lab_A123
hide labels, dl_A123
pseudoatom lab_A125, pos=[26.693, -85.905, 11.519], label="ARG125"
pseudoatom lab_A115, pos=[61.461, -83.217, 21.956], label="MET115"
distance dl_A115, (5GRJ and chain A and resi 115 and name CA), lab_A115
hide labels, dl_A115
pseudoatom lab_A117, pos=[38.538, -81.175, 19.295], label="SER117"
pseudoatom lab_A78, pos=[37.601, -86.931, 53.481], label="HIS78"
distance dl_A78, (5GRJ and chain A and resi 78 and name CA), lab_A78
hide labels, dl_A78
set label_color, black, lab_*

set_view (\
   0.7960818, -0.6051045, 0.0101109,\
   0.0198532, 0.0094137, -0.9997586,\
   0.6048632, 0.7960904, 0.0195073,\
   0.0000000, 0.0000000, -148.8448181,\
   25.4879608, -80.6812973, 22.9356422,\
   117.3503876, 180.3392487, -20.0000000 )

set ray_trace_fog, 0
ray 2800, 2100
png pymol_groundtruth/figures/5GRJ_twostream.png, dpi=350, ray=0
save pymol_groundtruth/figures/5GRJ_twostream.pse
