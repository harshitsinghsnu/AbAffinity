# 5Y9J -- Two-stream -- GT binding residues coloured by IG attribution
reinitialize
load pymol_groundtruth/pdb/pdb5y9j.ent, 5Y9J
remove not polymer
remove 5Y9J and not chain H+L+A
hide everything
bg_color white
set ray_opaque_background, 1
set ray_shadows, 0
set ray_trace_mode, 1
set ray_trace_color, grey50
set antialias, 2
set cartoon_fancy_helices, 1
set cartoon_transparency, 0.25
set cartoon_side_chain_helper, 1
set stick_radius, 0.30
set sphere_scale, 0.34
set valence, 0
set label_size, 15
set label_font_id, 7
set label_color, black
set label_outline_color, white
set label_bg_color, white
set label_bg_transparency, 0.25
set float_labels, 1
set dash_color, grey30
set dash_width, 1.6
set dash_gap, 0.40
set dash_length, 0.40
set dash_radius, 0.045
set dash_round_ends, 0
show cartoon
color 0xC4E7BD, chain H
color 0xCBDEF0, chain L
color 0xCECEE5, chain A
show sticks, (5Y9J and chain H and resi 31) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 31) and name CA
color 0x37A055, 5Y9J and chain H and resi 31
show sticks, (5Y9J and chain H and resi 54) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 54) and name CA
color 0x3CA659, 5Y9J and chain H and resi 54
show sticks, (5Y9J and chain H and resi 55) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 55) and name CA
color 0x30994F, 5Y9J and chain H and resi 55
show sticks, (5Y9J and chain H and resi 57) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 57) and name CA
color 0x4EB163, 5Y9J and chain H and resi 57
show sticks, (5Y9J and chain H and resi 101) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 101) and name CA
color 0x17803C, 5Y9J and chain H and resi 101
show sticks, (5Y9J and chain H and resi 102) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 102) and name CA
color 0x04702F, 5Y9J and chain H and resi 102
show sticks, (5Y9J and chain H and resi 103) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 103) and name CA
color 0x006629, 5Y9J and chain H and resi 103
show sticks, (5Y9J and chain H and resi 104) and (sidechain or name CA)
show spheres, (5Y9J and chain H and resi 104) and name CA
color 0x006127, 5Y9J and chain H and resi 104
show sticks, (5Y9J and chain L and resi 95) and (sidechain or name CA)
show spheres, (5Y9J and chain L and resi 95) and name CA
color 0x2C7CBB, 5Y9J and chain L and resi 95
show sticks, (5Y9J and chain L and resi 31) and (sidechain or name CA)
show spheres, (5Y9J and chain L and resi 31) and name CA
color 0x4795C8, 5Y9J and chain L and resi 31
show sticks, (5Y9J and chain L and resi 28) and (sidechain or name CA)
show spheres, (5Y9J and chain L and resi 28) and name CA
color 0x4896C8, 5Y9J and chain L and resi 28
show sticks, (5Y9J and chain L and resi 50) and (sidechain or name CA)
show spheres, (5Y9J and chain L and resi 50) and name CA
color 0x8BC0DD, 5Y9J and chain L and resi 50
show sticks, (5Y9J and chain A and resi 162) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 162) and name CA
color 0x5B3595, 5Y9J and chain A and resi 162
show sticks, (5Y9J and chain A and resi 163) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 163) and name CA
color 0x5A3495, 5Y9J and chain A and resi 163
show sticks, (5Y9J and chain A and resi 206) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 206) and name CA
color 0x6E59A7, 5Y9J and chain A and resi 206
show sticks, (5Y9J and chain A and resi 231) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 231) and name CA
color 0x7A72B4, 5Y9J and chain A and resi 231
show sticks, (5Y9J and chain A and resi 233) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 233) and name CA
color 0x9C98C7, 5Y9J and chain A and resi 233
show sticks, (5Y9J and chain A and resi 240) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 240) and name CA
color 0x7568AF, 5Y9J and chain A and resi 240
show sticks, (5Y9J and chain A and resi 242) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 242) and name CA
color 0x7261AB, 5Y9J and chain A and resi 242
show sticks, (5Y9J and chain A and resi 265) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 265) and name CA
color 0x5D3897, 5Y9J and chain A and resi 265
show sticks, (5Y9J and chain A and resi 266) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 266) and name CA
color 0x776BB0, 5Y9J and chain A and resi 266
show sticks, (5Y9J and chain A and resi 224) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 224) and name CA
color 0x6A51A3, 5Y9J and chain A and resi 224
show sticks, (5Y9J and chain A and resi 222) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 222) and name CA
color 0x8F8BC1, 5Y9J and chain A and resi 222
show sticks, (5Y9J and chain A and resi 225) and (sidechain or name CA)
show spheres, (5Y9J and chain A and resi 225) and name CA
color 0x776BB0, 5Y9J and chain A and resi 225
select interface, 5Y9J and ((chain H and resi 31+54+55+57+101+102+103+104) or (chain L and resi 95+31+28+50) or (chain A and resi 162+163+206+231+233+240+242+265+266+224+222+225))
deselect
set light_count, 2
set specular, 0.15
set ambient, 0.45

pseudoatom lab_H31, pos=[-7.006, -1.418, 72.895], label="ASN31"
distance dl_H31, (5Y9J and chain H and resi 31 and name CA), lab_H31
hide labels, dl_H31
pseudoatom lab_H54, pos=[18.256, 2.232, 68.565], label="MET54"
pseudoatom lab_H55, pos=[5.342, -4.245, 90.667], label="PHE55"
distance dl_H55, (5Y9J and chain H and resi 55 and name CA), lab_H55
hide labels, dl_H55
pseudoatom lab_H57, pos=[25.816, -0.297, 73.871], label="THR57"
pseudoatom lab_H101, pos=[-2.488, -4.383, 78.771], label="ASP101"
distance dl_H101, (5Y9J and chain H and resi 101 and name CA), lab_H101
hide labels, dl_H101
pseudoatom lab_H102, pos=[27.397, -0.81, 59.303], label="LEU102"
pseudoatom lab_H103, pos=[24.974, -3.478, 58.033], label="LEU103"
pseudoatom lab_H104, pos=[23.353, -0.989, 55.656], label="LEU104"
pseudoatom lab_L95, pos=[35.257, -0.775, 64.7], label="ASN95"
pseudoatom lab_L31, pos=[38.438, 2.942, 53.073], label="TYR31"
pseudoatom lab_L28, pos=[44.654, -2.239, 53.988], label="ARG28"
pseudoatom lab_L50, pos=[35.699, 4.49, 46.935], label="LYS50"
pseudoatom lab_A162, pos=[14.045, -8.498, 102.458], label="SER162"
distance dl_A162, (5Y9J and chain A and resi 162 and name CA), lab_A162
hide labels, dl_A162
pseudoatom lab_A163, pos=[30.511, -7.838, 65.077], label="TYR163"
pseudoatom lab_A206, pos=[-9.538, -8.447, 66.781], label="TYR206"
distance dl_A206, (5Y9J and chain A and resi 206 and name CA), lab_A206
hide labels, dl_A206
pseudoatom lab_A231, pos=[25.322, -9.423, 52.081], label="ARG231"
pseudoatom lab_A233, pos=[-16.047, -15.234, 54.725], label="ILE233"
distance dl_A233, (5Y9J and chain A and resi 233 and name CA), lab_A233
hide labels, dl_A233
pseudoatom lab_A240, pos=[8.882, -13.03, 68.631], label="LEU240"
pseudoatom lab_A242, pos=[-11.188, -19.916, 60.56], label="ASN242"
distance dl_A242, (5Y9J and chain A and resi 242 and name CA), lab_A242
hide labels, dl_A242
pseudoatom lab_A265, pos=[2.857, -11.51, 84.547], label="ARG265"
distance dl_A265, (5Y9J and chain A and resi 265 and name CA), lab_A265
hide labels, dl_A265
pseudoatom lab_A266, pos=[10.501, -10.436, 96.466], label="GLU266"
distance dl_A266, (5Y9J and chain A and resi 266 and name CA), lab_A266
hide labels, dl_A266
pseudoatom lab_A224, pos=[57.028, -0.324, 40.346], label="LEU224"
distance dl_A224, (5Y9J and chain A and resi 224 and name CA), lab_A224
hide labels, dl_A224
pseudoatom lab_A222, pos=[41.182, 0.334, 16.575], label="ASP222"
distance dl_A222, (5Y9J and chain A and resi 222 and name CA), lab_A222
hide labels, dl_A222
pseudoatom lab_A225, pos=[72.909, -1.161, 64.112], label="SER225"
distance dl_A225, (5Y9J and chain A and resi 225 and name CA), lab_A225
hide labels, dl_A225
set label_color, black, lab_*

set_view (\
   0.8212981, 0.5362101, 0.1948032,\
   0.1761466, 0.0864357, -0.9805618,\
   -0.5426250, 0.8396474, -0.0234621,\
   0.0000000, 0.0000000, -187.2737579,\
   27.4395847, -3.2070789, 59.5378990,\
   147.6480560, 226.8994598, -20.0000000 )

set ray_trace_fog, 0
ray 2800, 2100
png pymol_groundtruth/figures/5Y9J_twostream.png, dpi=350, ray=0
save pymol_groundtruth/figures/5Y9J_twostream.pse
